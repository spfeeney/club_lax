...
urlpatterns = patterns('',
                       ...
                       url(r'^question/(?P<question_>\d+)/answer/delete/(?P<answer_pk>\d+)/$', login_required(AnswerDeleteView.as_view()), name='answer_delete'),
                       url(r'^user/(?P<slug>\w+)/$', login_required(UserDetailView.as_view()), name='user_detail'),
                       url(r'^roster/$', Roster.as_view(), name='roster'),
